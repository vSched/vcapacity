...
bool verbose = false;
double milliseconds_totick_factor =
    static_cast<double>(sysconf(_SC_CLK_TCK)) / 1000.0;
...
int initialized = 0;
int profiler_iter = -1;
chrono::time_point<chrono::_V2::system_clock,
                   chrono::_V2::system_clock::duration>
    endtime;
chrono::time_point<chrono::_V2::system_clock,
                   chrono::_V2::system_clock::duration>
    hard_endtime;
...
double calculate_ema(int decay_len, double &ema_help, double prev_ema,
                     double new_value) {
...
// parameter utilities
string_view get_option(const vector<string_view> &args,
...
bool has_option(const vector<string_view> &args,
                const string_view &option_name) {
  for (auto it = args.begin(), end = args.end(); it != end; ++it) {
    if (*it == option_name)
      return true;
...
void move_thread_to_low_prio(pid_t tid) {
  ...
  if (!ofs) {
    cerr << "Low priority cGroup cannot be found - Try running the setup "
            "script\n";
    return;
  }
  ofs << tid << "\n";
...
void move_thread_to_high_prio(pid_t tid) {
  ...
  ofstream ofs(path, ios_base::app);
  if (!ofs) {
    cerr << "High priority cGroup cannot be found - Try running the setup "
            "script\n";
    return;
  }
  ofs << tid << "\n";
...
void move_current_thread() {
  ...
  ofstream ofs(path, ios_base::app);
  if (!ofs) {
    cerr << "Something is wrong with cgroup process- check linux version is "
            "right\n";
    return;
  }
  ofs << tid << "\n";
...
void get_cpu_information(int cpunum, vector<raw_data> &data_arr,
                         vector<thread_args *> thread_arg) {
  ifstream f("/proc/vcap_info");
  if (!f.is_open()) {
    cerr << "Check to see if kernel modules have been installed - vsched is "
            "necessary. "
         << endl;
    return;
  }
  ...
  for (int i = 0; i < cpunum && getline(stat_file, line); i++) {
    ...
    if (cpu == "cpu" + to_string(i)) {
      ...
      cerr << "Unexpected CPU identifier: " << cpu << " for index " << i
           << endl;
    }
  }
...
void set_arguments(const vector<string_view> &arguments) {
  ...
  auto set_option_value = [&](const string_view &option, int &target) {
    if (auto value = get_option(arguments, option); !value.empty()) {
      try {
        ...
        throw invalid_argument(string("Invalid argument for option ") +
                               string(option));
      } catch (const out_of_range &) {
        throw out_of_range(string("Out of range argument for option ") +
                           string(option));
      }
    }
  };
  ...
  set_option_value("-o", optimization_enabled);
...
void process_raw_capacity(vector<profiled_data> &data) {
  ...
  // raw capacity is meant to capture ARM-big-little architetcture not
  // necessarily frequency fluctuations, so our rounding is quite aggressive
  for (profiled_data &pd : data) {
  ...
  // if the last 5 raw capacities are stable, we can decrease heavy profiling
  // frequency for (profiled_data& pd : data) {
...
void get_finalized_data(int numthreads, double profile_time,
                        vector<raw_data> &data_begin,
                        vector<raw_data> &data_end,
                        vector<profiled_data> &result_arr,
                        vector<thread_args *> thread_arg) {
  ...
  for (int i = 0; i < numthreads; i++) {
    ...
    double used_time =
        (data_end[i].use_time - data_begin[i].use_time) * 10000000;
    if (!(profile_time) == 0) {
      double capacity_perc_1 =
          ((double)(used_time) / (used_time + stolen_pass));
    ...
    add_to_history(result_arr[i].capacity_perc_hist,
                   result_arr[i].capacity_perc);
    ...
    result_arr[i].latency_ema =
        calculate_ema(decay_length, result_arr[i].latency_ema_a,
                      result_arr[i].latency_ema, result_arr[i].latency);
    result_arr[i].capacity_perc_ema = calculate_ema(
        decay_length, result_arr[i].capacity_perc_ema_a,
        result_arr[i].capacity_perc_ema, result_arr[i].capacity_perc);
    result_arr[i].capacity_perc_stddev =
        calculate_std_dev(result_arr[i].capacity_perc_hist);
  };

  if (profiler_iter % heavy_profile_interval == 0) {
    process_raw_capacity(result_arr);
    for (int i = 0; i < numthreads; i++) {
      add_to_history(result_arr[i].capacity_adj_hist,
                     result_arr[i].capacity_adj);
      result_arr[i].capacity_adj_stddev =
          calculate_std_dev(result_arr[i].capacity_adj_hist);
...
void print_result(int cpunum, vector<profiled_data> &result,
                  vector<thread_args *> thread_arg) {
  for (int i = 0; i < cpunum; i++) {
    cout << "CPU:" << i << " TID:" << thread_arg[i]->tid << endl;
    cout << "Capacity Perc:" << result[i].capacity_perc
         << ":Latency:" << result[i].latency
         << ":Preempts:" << result[i].preempts
         << ":Capacity Raw:" << result[i].capacity_adj << endl;
    cout << ":Cperc stddev:" << result[i].capacity_perc_stddev
         << ":Max latency:" << result[i].max_latency;
    cout << ":Cperc ema: " << result[i].capacity_perc_ema
         << ":Latency EMA:" << result[i].latency_ema << endl
         << endl;
  }
  ...
  cout << "hardtimestamp: "
       << (bt.tm_min * 60000 + bt.tm_sec * 1000 + ms.count()) << endl;
  cout << "--------------" << endl;
}
...
void give_to_kernel(int cpunum, vector<profiled_data> &result_arr) {
  ...
  for (int i = 0; i < cpunum; i++) {
    capacity_res =
        capacity_res +
        __cxx11::to_string((int)round(result_arr[i].capacity_perc_ema * 1024)) +
        ";";
  }
  ...
  for (int i = 0; i < cpunum; i++) {
    latency_res = latency_res +
                  __cxx11::to_string((int)round(result_arr[i].latency_ema)) +
                  ";";
  }
...
void ban_vcpus(vector<profiled_data> &data_arr) {
  ...
  for (int i = 0; i < num_threads; i++) {
    if (vtop_banlist[i] != 1 &&
        (data_arr[i].capacity_perc_ema > straggler_cutoff)) {
      bans += to_string(i) + ",";
    }
  }
  if (!bans.empty()) {
...
bool preemptive_leave(vector<raw_data> &data_begin,
                      vector<raw_data> &data_end) {
  int max_preempts = -2;
  ...
  for (int i = 0; i < num_threads; i++) {
    ...
    maxed_capacity =
        ((data_end[i].steal_time - data_begin[i].steal_time) < 1000000);
...
void do_small_profile(vector<raw_data> &data_begin, vector<raw_data> &data_end,
                      vector<thread_args *> thread_arg) {
  while (true) {
    ...
    if (preemptive_leave(data_begin, data_end) ||
        chrono::high_resolution_clock::now() > endtime) {
      endtime = chrono::high_resolution_clock::now();
      break;
    }
  }
...
void do_profile(vector<raw_data> &data_end, vector<thread_args *> thread_arg) {
  ...
  while (true) {
    // If the last interval was heavy, move the threads to low priority. If
    // interval is less then 2, obviously special workload.
    if (!(heavy_profile_interval < 2) &&
        ((profiler_iter - 1) % heavy_profile_interval == 0)) {
      for (int i = 0; i < num_threads; i++) {
    ...
    // We want to set the endtime and get data immediately after the threads
    // have woken up in order to minimize innacuracy, this is to keep threads
    // waiting
    endtime = chrono::high_resolution_clock::now() +
    ...
    endtime = chrono::high_resolution_clock::now() +
              std::chrono::milliseconds(profile_time);
    get_cpu_information(num_threads, data_begin, thread_arg);
    // TODO-sleep every x ms and wake up to see if it's now(potentially)try nano
    // sleep? (do some testing) Wait for processors to finish profiling
    ...
    get_finalized_data(num_threads, test, data_begin, data_end, result_arr,
                       thread_arg);
...
vector<thread_args *> setup_threads(vector<pthread_t> &thread_array,
                                    vector<raw_data> &data_end) {
  cpu_set_t cpuset;
  ...
  // we need to make sure that all the threads have fetched the thread ID before
  // we go into whatever computation
  bool allset = true;
  while (true) {
    allset = true;
    for (int i = 0; i < num_threads; i++) {
...
int main(int argc, char *argv[]) {
  int num_cpus =
      sysconf(_SC_NPROCESSORS_ONLN); // Get the number of online processors
  vtop_banlist.resize(num_cpus, 0);
  // the threads need to be moved to root level cgroup before they can be
  // distributed to high/low cgroup
  move_current_thread();
  ...
  // note that this needs to be here because the computations and the main
    ...
    *args->addition_calc = addition_calculator;
    if (heavy_interval) {
      ...
      double test = static_cast<double>(timespec_diff_to_ns(&start, &end)) /
                    static_cast<double>(timespec_diff_to_ns(&lstart, &lend));
